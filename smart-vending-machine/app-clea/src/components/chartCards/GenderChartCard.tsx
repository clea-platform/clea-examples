...
const GenderChartCard: React.FC<ChartCardProps> = ( { data } ) => {
        ...
        genders.forEach((g, gindex) => {
            ...
            if (currentDataType === DataType.UNITS) {
                Object.entries(units).forEach(([key, value], index) => {        
                    choicesPoints[gindex].push({label: key, value})
                })
            } else {
    ...
    return (
        <div className="card-custom">
            ...
            <div className="row">
                {dataset.length && <BarChart datasets={dataset} measure={currentDataType === DataType.REVENUES ? "€" : ""} />}
            </div>
        </div>
    );
};
